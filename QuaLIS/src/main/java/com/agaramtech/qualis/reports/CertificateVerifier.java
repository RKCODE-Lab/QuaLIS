...
public final class CertificateVerifier
{
            ...
            throws CertificateVerificationException
    {
        try
        {
            ...
            if (downloadSize > 0)
            {
                LOGGER.info("CA issuers: " + downloadSize + " downloaded certificate(s) are new");
            }
            ...
            LOGGER.info("Certification chain verified successfully up to this root: " +
            ...
            GeneralSecurityException, IOException
    {
        ...
        if (ocspURL != null)
        {
            ...
            catch (IOException | OCSPException ex)
            {
                ...
                LOGGER.warn("Exception trying OCSP, will try CRL", ex);
                LOGGER.warn("Certificate# to check: " + cert.getSerialNumber().toString(16));
                CRLVerifier.verifyCertificateCRLs(cert, signDate, additionalCerts);
            }
        }
        else
        {
            LOGGER.info("OCSP not available, will try CRL");
    ...
    public static boolean isSelfSigned(X509Certificate cert) throws GeneralSecurityException
    {
        ...
        catch (SignatureException | InvalidKeyException | IOException ex)
        {
            ...
            LOGGER.debug("Couldn't get signature information - returning false", ex);
            return false;
        }
    }
    ...
    public static Set<X509Certificate> downloadExtraCertificates(X509Extension ext)
    {
        ...
        {
            LOGGER.warn("ASN1Sequence expected, got " + asn1Prim.getClass().getSimpleName());
            return resultSet;
        }
        ...
        while (objects.hasMoreElements())
        {
            ...
            {
                LOGGER.warn(urlString + " failure: " + ex.getMessage(), ex);
            }
        ...
        LOGGER.info("CA issuers: Downloaded " + resultSet.size() + " certificate(s) total");
        return resultSet;
    }
            ...
            throws RevokedCertificateException, IOException, OCSPException, CertificateVerificationException
    {
        ...
        LOGGER.info("OCSP check successful");
        ...
        if (ocspResponderCertificate.getExtensionValue(OCSPObjectIdentifiers.id_pkix_ocsp_nocheck.getId()) != null)
        {
            ...
            LOGGER.info("Revocation check of OCSP responder certificate skipped (id-pkix-ocsp-nocheck is set)");
            return;
        }

        if (ocspHelper.getCertificateToCheck().equals(ocspResponderCertificate))
        {
            LOGGER.info("OCSP responder certificate is identical to certificate to check");
            return;
        }

        LOGGER.info("Check of OCSP responder certificate");
        ...
        for (X509CertificateHolder certHolder : basicResponse.getCerts())
        {
            ...
            catch (CertificateException ex)
            {
                ...
                LOGGER.error("Certificate processing failed", ex);
            }
        }
        CertificateVerifier.verifyCertificate(ocspResponderCertificate, additionalCerts2, true, now);
        LOGGER.info("Check of OCSP responder certificate done");
    }
}
