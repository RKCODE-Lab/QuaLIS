...
{
	private static final Logger LOGGER = LoggerFactory.getLogger(OcspHelper.class);
            ...
            throws OCSPException, RevokedCertificateException, IOException
    {
        ...
        if (basicResponse != null)
        {
            ...
            if (status instanceof RevokedStatus)
            {
                ...
                LOGGER.info("The certificate was revoked after signing by OCSP " + ocspUrl + 
        ...
        try
        {
                ...
                responseCode == HttpURLConnection.HTTP_SEE_OTHER)
            {
                    ...
                    urlString.substring(7).equals(location.substring(8)))
                {
                	...
                	LOGGER.info("redirection to " + location + " followed");
                    return performRequest(location);
                }
                else
                {
                	LOGGER.info("redirection to " + location + " ignored");
                }
            }
    ...
    public void verifyRespStatus(OCSPResp resp) throws OCSPException
    {
        ...
        if (resp != null)
        {
            ...
            case OCSPResponseStatus.INTERNAL_ERROR:
                ...
                LOGGER.error("An internal error occurred in the OCSP Server!");
                break;
            case OCSPResponseStatus.MALFORMED_REQUEST:
                ...
                LOGGER.error("Your request did not fit the RFC 2560 syntax!");
                break;
            case OCSPResponseStatus.SIG_REQUIRED:
                statusInfo = "SIG_REQUIRED";
                LOGGER.error("Your request was not signed!");
                break;
            case OCSPResponseStatus.TRY_LATER:
                statusInfo = "TRY_LATER";
                LOGGER.error("The server was too busy to answer you!");
                break;
            case OCSPResponseStatus.UNAUTHORIZED:
                statusInfo = "UNAUTHORIZED";
                LOGGER.error("The server could not authenticate you!");
                break;
            case OCSPResponseStatus.SUCCESSFUL:
                break;
            default:
                statusInfo = "UNKNOWN";
                LOGGER.error("Unknown OCSPResponse status code! " + status);
            }
        }
