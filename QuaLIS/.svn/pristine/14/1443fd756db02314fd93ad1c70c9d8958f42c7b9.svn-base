...
public final class CRLVerifier
{
            ...
            throws CertificateVerificationException, RevokedCertificateException
    {
        try
        {
            ...
            {
            	LOGGER.info("Checking distribution point URL: " + crlDistributionPointsURL);
                ...
                catch (IOException | GeneralSecurityException | CertificateVerificationException | NamingException ex)
                {
                	...
                	LOGGER.warn("Caught " + ex.getClass().getSimpleName() + " downloading CRL, will try next distribution point if available");
                ...
                {
                	LOGGER.error("CRL not yet valid, thisUpdate is " + crl.getThisUpdate());
                }
                if (crl.getNextUpdate().before(now))
                {
                	LOGGER.error("CRL no longer valid, nextUpdate is " + crl.getNextUpdate());
                }

                if (!crl.getIssuerX500Principal().equals(cert.getIssuerX500Principal()))
                {
                	LOGGER.info("CRL issuer certificate is not identical to cert issuer, check needed");
                    CertificateVerifier.verifyCertificate(crlIssuerCert, mergedCertSet, true, now);
                    LOGGER.info("CRL issuer certificate checked successfully");
                }
                else
                {
                	LOGGER.info("CRL issuer certificate is identical to cert issuer, no extra check needed");
                }
                ...
                throws RevokedCertificateException
    {
        ...
        {
        	LOGGER.info("The certificate was revoked after signing by CRL " +
                    crlDistributionPointsURL + " on " + revokedCRLEntry.getRevocationDate());
        }
        else
        {
        	LOGGER.info("The certificate was not revoked by CRL " + crlDistributionPointsURL);
        }
    }
            ...
            throws IOException
    {
        ...
        {
        	LOGGER.warn("CRL distribution points for certificate subject " +
